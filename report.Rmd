...
```{r}
f_a_prior <- make_f_a_prior(0.1, 0.7)
f_a <- seq(0, 1, length.out = 100)
plot(f_a, f_a_prior(f_a), type = 'l', 
     xlab = expression(f[a]), ylab = expression(pi*(f[a])))
```

As a simple model for now we assume that there is only drift going on along edges, and no mutations, which makes the $m$ and $v$ functions simple. I am actually using a normal distribution here instead of a beta distribution **because I think the formula on my whiteboard is wrong -- I don't get anything meaninful out of using it**.

```{r}
m <- function(f_a, t_i) f_a
v <- function(f_a, t_i) (1 - exp(-t_i)) * f_a * (1 - f_a)
conditional_f_i <- function(f_i, t_i, f_a) dnorm(f_i, m(f_a, t_i), v(f_a, t_i))
```

```{r}
f_a <- 0.2 ; t_i <- 0.2
f_i <- seq(0, 1, length.out = 500)
plot(f_i, conditional_f_i(f_i, t_i, f_a), type = 'l', 
     xlab = expression(f[i]), ylab = expression(p*(f[i])))
abline(v = f_a, col = "red")
```

Since I am using a normal distribution here, I can't just do the magic that conjugate priors do, so I have to compute the probability of the observed counts using numerical integration. This should be fixed when we use the right formula.

```{r}
conditional_k_i <- function(k_i, n_i, t_i, f_a) {
  f <- function(f_i) dbinom(k_i, n_i, f_i) * conditional_f_i(f_i, t_i, f_a)
...
```{r}
simulate_counts <- function(ns, ts, f_a = NULL, a = 0.1, b = 0.7) {
  if (is.null(f_a)) f_a <- rbeta(1, a, b)
  
  fs <- unlist(Map(rnorm, 1, m(f_a, ts), v(f_a, ts)))
